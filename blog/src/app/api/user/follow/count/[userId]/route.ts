...
export async function GET(req: NextRequest): Promise<NextResponse> {
  console.debug("Received GET request for follow counts.");
  ...
  try {
    ...
    console.debug("UserId to fetch:", userId);

    await dbConnect();
    console.debug("Database connection established.");

    // Retrieve the user and populate followers and following fields
    ...
    console.error("Error retrieving follow counts:", error);
    return NextResponse.json(
      new ApiError(500, "Error retrieving follow counts"),
      { status: 500 }
    );
  }
}
