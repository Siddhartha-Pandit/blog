...
const Dropzone: React.FC<{
  ...
  return (
    <div
      ...
      className={`relative w-full h-48 border-2 border-dashed rounded-md flex flex-col items-center justify-center cursor-pointer transition-colors ${
...
export const CoverImageModal = () => {
  ...
  const handleUpload = async (file: File) => {
    ...
    try {
      ...
      const updateRes = await fetch("/api/blog/update", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      if (!updateRes.ok) throw new Error("Failed to update document cover");
