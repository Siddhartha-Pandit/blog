...
const UserItem: React.FC = () => {
  ...
  const { data: session, status } = useSession(); // ✅ Directly use NextAuth session
  ...
  // ✅ Use NextAuth's signOut instead of custom logout API
          ...
          <div className="flex items-center gap-x-2 max-w-[220px]">
            ...
            <div className="flex items-center gap-x-1 min-w-0">
              <span className="text-start font-medium line-clamp-1 truncate">
                {user?.name ? `${user.name}'s Jotion` : "Your Jotion"}
              </span>
